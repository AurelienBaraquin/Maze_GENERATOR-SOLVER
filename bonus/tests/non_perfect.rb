...
def maze_is_valid(buffer, width, height)
    size = 0
    has_dead_ends = false
    for i in 0...buffer.size
        size += 1
        if buffer[i] == ROAD && buffer[i + 1] == ROAD && buffer[i + width + 1] == ROAD && buffer[i + width + 2] == ROAD
            has_dead_ends = true
        end
    end
    unless size - 1 == (width * height + (height)) - 1 && has_dead_ends == true
        return false
    end
    return true
end
# END FUNCTION (maze_is_not_valid)
...
while x <= MAX
    ...
    buffer = `./generator/generator #{x.to_s} #{y.to_s}`
    for i in 0..buffer.size
        buffer[i] = WALL if buffer[i] == "X"
        buffer[i] = ROAD if buffer[i] == "*"
    end
    if maze_is_vali d(buffer, x, y)
